  ...
  const stream = new ReadableStream({
    async start(controller) {
      ...
      try {
        ...
        const deferredChartCalls = []; // Queue LLM-requested create_chart calls for post-answer execution
        ...
        for (let i = 0; i < MAX_TOOL_ITERATIONS; i++) {
          ...
          if (!response.tool_calls || response.tool_calls.length === 0) {
             ...
             */
            const runDeferredAutoCharts = async () => {
            ...
            // Execute LLM-requested chart calls after final answer is already shown
            if (deferredChartCalls.length > 0) {
              const chartTool = tools.find((t) => t.name === "create_chart");
              if (chartTool) {
                for (const deferred of deferredChartCalls) {
                  sendEvent({
                    type: "tool_start",
                    tool: "create_chart",
                    message: "📊 Creating chart...",
                  ...
                  try {
                    const chartResult = await chartTool.invoke(deferred.args || {});
                    sendEvent({
                      type: "tool_end",
                      tool: "create_chart",
                      message: "📊 Creating chart ✅",
                    ...
                    console.error("❌ Deferred create_chart error:", err.message);
                    sendEvent({ type: "tool_end", tool: "create_chart", message: "📊 Chart skipped" });
                  }
                }
              }
            ...
            // Deferred: chart creation happens after final answer reaches user
            await runDeferredAutoCharts();
          ...
          const toolResults = await Promise.all(
            ...
            // Move chart generation off the critical path:
            // acknowledge to LLM now, execute after final answer.
            if (toolCall.name === "create_chart") {
              return {
                toolCall,
                normalizedArgs,
                fullResult: JSON.stringify({ deferred: true }),
                compactResult:
                  "Chart generation deferred until after final answer is delivered.",
                deferred: true,
              };
            }
          ...
          for (const { toolCall, normalizedArgs, fullResult, compactResult, deferred } of toolResults) {
            calledTools.push(toolCall.name);

            if (deferred && toolCall.name === "create_chart") {
              deferredChartCalls.push({ toolCall, args: normalizedArgs });
            }
